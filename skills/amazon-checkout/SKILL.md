...
Place a new Amazon order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct Amazon product page URL (on amazon.com)
...
Place a Amazon order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
## Example: Place a Amazon Order (API Key)
...
Amazon supports both consumer and (where enabled) business catalogs, plus a German storefront (amazon.de). Use Managed Accounts to order with a Prime or Business account for member pricing and faster shipping.

## Managed Accounts (Retailer Credentials)

Managed accounts let users supply their own Amazon login credentials instead of relying on Zinc's default accounts. This is useful for users who want more control over order processing or need account-specific settings (membership pricing, business pricing, etc.).
...
- **Email forwarding:** Each managed account gets a dedicated Zinc email address for receiving retailer verification/2FA codes. Configure your email provider to forward Amazon emails to this address.

### Create Managed Account — `POST /managed-accounts`

Register Amazon credentials with Zinc.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string | Yes | Amazon account email |
| `password` | string | No | Amazon account password (encrypted at rest) |
| `retailer` | string | No | Retailer identifier (`"amazon"`) |
...
- **TOTP 2FA:** If the Amazon account has 2FA enabled, provide the TOTP secret key (the 64-character base32 string, not the rotating 6-digit code).
- **Email forwarding:** Forward only amazon.com emails to the Zinc forwarding address — avoid forwarding all mail.
- **Best practice:** Use a dedicated Amazon account for Zinc to avoid conflicts with personal orders.
