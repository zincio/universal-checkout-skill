...
Place a new Lowe's order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct Lowe's product page URL (on lowes.com)
...
Place a Lowe's order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
async with Client(methods=[method]) as client:
    response = await client.post(
        ...
        json={
            "products": [{"url": "https://www.lowes.com/pd/5013499741", "quantity": 1}],
            "max_price": 5000,
            "shipping_address": {
                "first_name": "Jane",
                "last_name": "Doe",
...
## Example: Place a Lowe's Order (API Key)
...
Lowe's orders are placed via guest checkout — no retailer login required.
