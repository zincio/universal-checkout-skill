...
Place a new Walmart order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct Walmart product page URL (on walmart.com)
...
Place a Walmart order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
## Example: Place a Walmart Order (API Key)
...
Walmart supports Managed Accounts — useful for Walmart+ members and order history under a specific account.

## Managed Accounts (Retailer Credentials)

Managed accounts let users supply their own Walmart login credentials instead of relying on Zinc's default accounts. This is useful for users who want more control over order processing or need account-specific settings (membership pricing, business pricing, etc.).
...
- **Email forwarding:** Each managed account gets a dedicated Zinc email address for receiving retailer verification/2FA codes. Configure your email provider to forward Walmart emails to this address.

### Create Managed Account — `POST /managed-accounts`

Register Walmart credentials with Zinc.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string | Yes | Walmart account email |
| `password` | string | No | Walmart account password (encrypted at rest) |
| `retailer` | string | No | Retailer identifier (`"walmart"`) |
...
- **TOTP 2FA:** If the Walmart account has 2FA enabled, provide the TOTP secret key (the 64-character base32 string, not the rotating 6-digit code).
- **Email forwarding:** Forward only walmart.com emails to the Zinc forwarding address — avoid forwarding all mail.
- **Best practice:** Use a dedicated Walmart account for Zinc to avoid conflicts with personal orders.
