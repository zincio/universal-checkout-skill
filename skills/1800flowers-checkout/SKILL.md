...
Place a new 1-800-Flowers order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct 1-800-Flowers product page URL (on 1800flowers.com)
...
Place a 1-800-Flowers order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
async with Client(methods=[method]) as client:
    response = await client.post(
        ...
        json={
            "products": [{"url": "https://www.1800flowers.com/product-name-12345", "quantity": 1}],
            "max_price": 5000,
            "shipping_address": {
                "first_name": "Jane",
                "last_name": "Doe",
...
## Example: Place a 1-800-Flowers Order (API Key)
...
1-800-Flowers orders are placed via guest checkout. Great for automating gifting — pair the order with a gift message in `metadata` if your workflow tracks one.
