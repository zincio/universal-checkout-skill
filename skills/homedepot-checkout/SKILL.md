...
Place a new The Home Depot order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct The Home Depot product page URL (on homedepot.com)
...
Place a The Home Depot order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
## Example: Place a The Home Depot Order (API Key)
...
Home Depot requires a buyer account (guest checkout is not supported) — supply credentials via a Managed Account.

## Managed Accounts (Retailer Credentials)

Managed accounts let users supply their own The Home Depot login credentials instead of relying on Zinc's default accounts. This is useful for users who want more control over order processing or need account-specific settings (membership pricing, business pricing, etc.).
...
- **Email forwarding:** Each managed account gets a dedicated Zinc email address for receiving retailer verification/2FA codes. Configure your email provider to forward The Home Depot emails to this address.

### Create Managed Account — `POST /managed-accounts`

Register The Home Depot credentials with Zinc.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string | Yes | The Home Depot account email |
| `password` | string | No | The Home Depot account password (encrypted at rest) |
| `retailer` | string | No | Retailer identifier (`"homedepot"`) |
...
- **TOTP 2FA:** If the The Home Depot account has 2FA enabled, provide the TOTP secret key (the 64-character base32 string, not the rotating 6-digit code).
- **Email forwarding:** Forward only homedepot.com emails to the Zinc forwarding address — avoid forwarding all mail.
- **Best practice:** Use a dedicated The Home Depot account for Zinc to avoid conflicts with personal orders.
