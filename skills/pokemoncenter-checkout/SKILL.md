...
curl -X POST https://api.zinc.com/orders \
  ...
  -d '{
    "products": [{ "url": "https://www.pokemoncenter.com/product/100-10-1234", "quantity": 1, "condition_in": ["New"] }],
    "max_price": 5000,
    "handling_days_max": 5,
    "shipping_address": {
      "first_name": "Jane",
...
Inventory is often limited-drop — set a sensible `max_price` and expect `product_out_of_stock` on sold-out items.

**Shipping:** Pokémon Center ships free on orders over $20. For cheaper items, shipping is added to the order total, so leave room for it in `max_price`. (Live terms: `GET /retailers`.)
