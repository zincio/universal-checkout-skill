...
Place a new Best Buy order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct Best Buy product page URL (on bestbuy.com)
...
Place a Best Buy order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
async with Client(methods=[method]) as client:
    response = await client.post(
        ...
        json={
            "products": [{"url": "https://www.bestbuy.com/site/apple-airpods-pro-2nd-generation/4900964.p", "quantity": 1}],
            "max_price": 5000,
            "shipping_address": {
                "first_name": "Jane",
                "last_name": "Doe",
...
## Example: Place a Best Buy Order (API Key)
...
Best Buy orders are placed via guest checkout — no retailer login required.
