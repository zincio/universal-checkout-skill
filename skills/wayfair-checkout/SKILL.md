...
Place a new Wayfair order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct Wayfair product page URL (on wayfair.com)
...
Place a Wayfair order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
async with Client(methods=[method]) as client:
    response = await client.post(
        ...
        json={
            "products": [{"url": "https://www.wayfair.com/furniture/pdp-w100123456.html", "quantity": 1}],
            "max_price": 5000,
            "shipping_address": {
                "first_name": "Jane",
                "last_name": "Doe",
...
## Example: Place a Wayfair Order (API Key)
...
Wayfair supports Managed Accounts for order history and account-specific pricing.

## Managed Accounts (Retailer Credentials)

Managed accounts let users supply their own Wayfair login credentials instead of relying on Zinc's default accounts. This is useful for users who want more control over order processing or need account-specific settings (membership pricing, business pricing, etc.).
...
- **Email forwarding:** Each managed account gets a dedicated Zinc email address for receiving retailer verification/2FA codes. Configure your email provider to forward Wayfair emails to this address.

### Create Managed Account — `POST /managed-accounts`

Register Wayfair credentials with Zinc.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string | Yes | Wayfair account email |
| `password` | string | No | Wayfair account password (encrypted at rest) |
| `retailer` | string | No | Retailer identifier (`"wayfair"`) |
...
- **TOTP 2FA:** If the Wayfair account has 2FA enabled, provide the TOTP secret key (the 64-character base32 string, not the rotating 6-digit code).
- **Email forwarding:** Forward only wayfair.com emails to the Zinc forwarding address — avoid forwarding all mail.
- **Best practice:** Use a dedicated Wayfair account for Zinc to avoid conflicts with personal orders.
