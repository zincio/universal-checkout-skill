...
Place a new eBay order using your API key. Requires `Authorization: Bearer $ZINC_API_KEY`.

**Required fields:**

- `products` — array of `{ url, quantity?, variant? }` objects
  - `url`: direct eBay product page URL (on ebay.com)
...
Place a eBay order using the Machine Payments Protocol. No API key required — payment is made inline via on-chain crypto.
...
## Example: Place a eBay Order (API Key)
...
eBay requires a buyer account (guest checkout is not supported) — supply credentials via a Managed Account. Buy It Now / fixed-price listings only.

## Managed Accounts (Retailer Credentials)

Managed accounts let users supply their own eBay login credentials instead of relying on Zinc's default accounts. This is useful for users who want more control over order processing or need account-specific settings (membership pricing, business pricing, etc.).
...
- **Email forwarding:** Each managed account gets a dedicated Zinc email address for receiving retailer verification/2FA codes. Configure your email provider to forward eBay emails to this address.

### Create Managed Account — `POST /managed-accounts`

Register eBay credentials with Zinc.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `email` | string | Yes | eBay account email |
| `password` | string | No | eBay account password (encrypted at rest) |
| `retailer` | string | No | Retailer identifier (`"ebay"`) |
...
- **TOTP 2FA:** If the eBay account has 2FA enabled, provide the TOTP secret key (the 64-character base32 string, not the rotating 6-digit code).
- **Email forwarding:** Forward only ebay.com emails to the Zinc forwarding address — avoid forwarding all mail.
- **Best practice:** Use a dedicated eBay account for Zinc to avoid conflicts with personal orders.
